...
B2 Pre-Released 未修改完成版本，B1 版见一版规则书对应文件
...
列车二车厢看上去像是一间教室，比列车本身应有的大小要大很多。但是窗外仍是之前的景象。教室内和普通教室类似的布置，在其中一个的桌面上端正地摆着一束白百合（莲实的桌椅），而在这个桌子前面的位置是如月的桌椅。从教室的后门离开就会到达列车的三车厢
...
- **黑板**上写着“祝贺毕业”。但是被擦掉了，仅能隐隐约约看到痕迹。黑板的凹槽内有一些粉笔。在二车厢中粉笔则散落在各处且折断了无法再使用。
...
**回忆·二车厢**中光线变得灰暗。莲实的椅子则被倒扣在桌子上。一起推到了角落。如月的桌面上写满了“杀人犯”之类的字眼。其他桌椅则远远地挤在一起。整个教室混乱不堪，仿佛台风过境一样。
- 在回忆中，玩偶的身体会恢复完整，并露出标签上的二维码。
  
<img src="./images/qrcode.png" width="150px" class="img"/>

  - 使用如月的手机扫码会显示如下信息：
    ```
    
    ```
  - 使用其他的手机扫码则会卡死或显示找不到网络
  - 在玩家刚读完消息，乱码就变成无数的“对不起”从屏幕中冒出来，直到填满整个房间将教室淹没。查看消息的少女需要进行体质检定，失败会因为窒息而损失 0/1D2 的 SAN 值。当玩家回过神时就回到了原本的车厢中。
## 第四部分：列车三车厢
